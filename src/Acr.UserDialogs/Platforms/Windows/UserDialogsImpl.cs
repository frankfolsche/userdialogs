...
namespace Acr.UserDialogs
{
    public class UserDialogsImpl : AbstractUserDialogs
    {
        ...
        {
            this.dispatcher = dispatcher ?? new Func<Action, Task>((x) =>
            {
                return Task.FromResult(DispatcherQueue.GetForCurrentThread().TryEnqueue(DispatcherQueuePriority.Normal ,() => x()));
            }
            );
            //this.dispatcher = dispatcher ?? new Func<Action, Task>((x) =>
            //{
        ...
        protected override IProgressDialog CreateDialogInstance(ProgressDialogConfig config) =>
            new ProgressDialog(config, dispatcher);


        protected virtual IDisposable DispatchAndDispose(object target, Action dispatch, Action dispose)
        {
